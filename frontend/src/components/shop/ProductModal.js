...
import Rating from "../tabProduct/ProductRating";
...
function ProductModal({ product, discountedPrice, finalProductPrice, finalDiscountedPrice, show, onHide, wishlistItem, compareItem }) {
  ...
  const [selectedProductColor, setSelectedProductColor] = useState(
    product.variation && product.variation.length >= 1 ? product.variation[0].color : ""
  );
  const [selectedProductSize, setSelectedProductSize] = useState(
    product.variation && product.variation.length >= 1 ? product.variation[0].size[0].name : ""
  );
  const [productStock, setProductStock] = useState(
    product.variation && product.variation.length >= 1 ? product.variation[0].size[0].stock : product.stock
  );
